...
export default function PostPage() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const res = await fetch(`https://mediajourney1-holewhpc.b4a.run/api/post/getposts?slug=${postSlug}`);
  ...
  useEffect(() => {
    try {
      const fetchRecentPosts = async () => {
        const res = await fetch(`https://mediajourney1-holewhpc.b4a.run/api/post/getposts?limit=3`);
        const data = await res.json();
        if (res.ok) {
